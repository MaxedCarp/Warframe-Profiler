	...
	async execute(interaction) {
		if (interaction.channel.id !== "923290922010177616"){
			...
			if (await exist) {
				...
				//const framel2 = framel.toLowerCase();
				...
				finalframel = uniq(finalframel.filter(i => obj.framelist?.indexOf(i) === -1));
				let nonexistentframel = uniq(framelsplit.filter(i => lower.indexOf(i.toLowerCase()) === -1));
				let alrlistedframel = framelist.filter(i => framelsplit.indexOf(i.toLowerCase()) !== -1);
				alrlistedframel = uniq(alrlistedframel.filter(i => obj.framelist?.indexOf(i) !== -1));
